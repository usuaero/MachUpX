...
class Airplane:
    ...
    def export_stp(self, filename, section_resolution=200):
        ...
        # Check extension
        if ".stp" not in filename and ".step" not in filename:
            raise IOError("{0} is not a .stp or .step file.".format(filename))

        # Write file
        with open(filename, 'w') as step_handle:

            # Header
            step_handle.write("""ISO-1030-21;
HEADER;
...
ENDSEC;\n""".format(filename))
            ...
            # Data overhead
            step_handle.write("""DATA;
#15 = SHAPE_DEFINITION_REPRESENTATION(#16, #101);
#16 = PRODUCT_DEFINITION_SHAPE('{0}', '', #18);
#18 = PRODUCT_DEFINITION('', '', #19, #20);
#19 = PRODUCT_DEFINITION_FORMATION('version1', '', #21);
#20 = PRODUCT_DEFINITION_CONTEXT('', #23, '');
#21 = PRODUCT('no1', '{0}', 'Generated by MachUpX', (#22));
#22 = PRODUCT_CONTEXT('', #23, 'aeronautics');
#23 = APPLICATION_CONEXT('AP 203');\n""".format(self.name))

            # Collect wing segment solids
            solid_index = 200
            solid_indices = []
            for _, segment in self.wing_segments.items():
                solid_indices.append(solid_index)
                solid_string, solid_index = segment.get_stp_string(solid_index, section_resolution=section_resolution)
                step_handle.write(solid_string)

            # Create shape representation
            step_handle.write("\n#101 = ADVANCED_BREP_SHAPE_REPRESENTATION('', ({0}), #103);\n".format(", ".join(["#"+str(ind) for ind in solid_indices])))
            step_handle.write("#103 = GEOMETRIC_REPRESENTATION_CONTEXT(3);\n")
            step_handle.write("ENDSEC;\nEND-ISO-10303-21;")
